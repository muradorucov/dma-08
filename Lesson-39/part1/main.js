...
const pagElem = document.querySelector("#pagination");
...
function createPagination(length) {
  pagElem.innerHTML = "";
  for (let i = 1; i <= Math.ceil(length / 9); i++) {
    pagElem.innerHTML += `
     <button class="bg-[blueviolet] w-[50px] h-[40px] text-[#fff] cursor-pointer" onclick="getPagination(${i})">${i}</button>
 `
  }
}

inputElem.addEventListener("input", () => {
  const filteredData = data.filter(product => product.name.toLowerCase()
    .includes(inputElem.value.toLowerCase().trim()) || product.dsc.toLowerCase()
      .includes(inputElem.value.toLowerCase().trim()));
  productsElem.innerHTML = ""
  printScreen(filteredData)
});


function printScreen(list, startPag = 0, endPag = 9) {
  globalData = list
  createPagination(list.length)
  totalData.innerHTML = list.length
  list.slice(startPag, endPag).forEach((product) => {
    productsElem.innerHTML += `
  <div class="rounded-sm shadow-md hover:shadow-[none] transition-[300ms]">
        <img
          src="${product.img}"
          ...
          >
        <div class="p-[20px_30px] pb-[30px]">
          <h3 class="my-[7px_15px] text-[22px]">${product.name}</h3>
...
function getPagination(num) {
  ...
  let end = 9 * num;
  productsElem.innerHTML = ""
  printScreen(globalData, start, end)
}
